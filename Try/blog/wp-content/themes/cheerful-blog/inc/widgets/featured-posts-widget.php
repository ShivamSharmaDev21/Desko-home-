<?php
if ( ! class_exists( 'Cheerful_Blog_Featured_Posts_Widget' ) ) {
	/**
	 * Adds Cheerful Blog Featured Posts Widget.
	 */
	class Cheerful_Blog_Featured_Posts_Widget extends WP_Widget {
		...
		public function __construct() {
			$featured_posts_widget = array(
				...
				'description' => __( 'Retrive Posts List Widgets', 'cheerful-blog' ),
			);
			parent::__construct(
				'cheerful_blog_featured_posts_widget',
				__( 'Adore Widget: Featured Posts Widget', 'cheerful-blog' ),
			...
			<div class="adore-widget-body">
				...
				if ( $query->have_posts() ) :
					while ( $query->have_posts() ) :
						$query->the_post();
						?>
						<div class="post-item post-list <?php echo esc_attr( has_post_thumbnail() ? 'has-image' : '' ); ?>">
		...
		public function form( $instance ) {
			...
			<p>
				<label for="<?php echo esc_attr( $this->get_field_id( 'offset' ) ); ?>"><?php esc_html_e( 'Number of posts to displace or pass over:', 'cheerful-blog' ); ?></label>
			...
			<p>
				<label for="<?php echo esc_attr( $this->get_field_id( 'category' ) ); ?>"><?php esc_html_e( 'Select the category to show posts:', 'cheerful-blog' ); ?></label>
				<select id="<?php echo esc_attr( $this->get_field_id( 'category' ) ); ?>" name="<?php echo esc_attr( $this->get_field_name( 'category' ) ); ?>" class="widefat" style="width:100%;">
					<?php
					$categories = cheerful_blog_get_post_cat_choices();
