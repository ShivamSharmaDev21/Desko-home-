						...
						<div class="page-header">
							...
							<div class="d-flex">
								<div class="justify-content-center">
									...
									<a href="{{url('add_desk_top')}}" class="btn btn-primary my-2 btn-icon-text">
									 Add Desk Top
									</a>
								</div>
							</div>
       ...
       $('.remove_alert').fadeOut();;
									 ...
									 <div class="col-lg-12 col-md-12">
										<div class="table-responsive">
											...
											@if(isset($desktops) && count($desktops)>0)
											 @foreach($desktops as $des)
										
												<tr>
												<td>{{$i}}</td>
												<td>{{$des->top_name}}</td>
											
												<td>{{$des->material_name}}</td>
												<td>{{$des->top_price}}</td>
												...
												<td><a href="{{url('edit_desk_top/'.$des->top_id)}}" class="btn btn-primary">Edit</a></td>
                <td>
                    <form action="{{url('delete_desk_top/'.$des->top_id)}}" method="POST">
                        @csrf
                        @method('DELETE')
		...
		<script>
		    function  change_status(top_id,str){
		      //  alert(user_id+'='+str);
		      $.ajax({
                url: "{{url('change_desk_top_status')}}",
                type: 'post',
                data: {
                    str:str,
                    top_id:top_id,
