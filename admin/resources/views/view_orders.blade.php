										...
										<div class="table-responsive" >
											<table id="example" class="table text-center table-hover border-t0 key-buttons text-nowrap w-100 display nowrap" >
											 ...
											 @foreach($orders as $order)
 												<tr>
												<td>{{$i}}</td>
												<td>{{$order['order_details']->fname.' '.$order['order_details']->lname}}</td>
												<td>{{$order['order_details']->order_id}}</td>
												<td>{{$order['order_details']->sub_total}}</td>
												<td>{{$order['order_details']->payment_id}}</td>
												<td>{{date('d-m-Y',strtotime($order['order_details']->created_at))}}</td>
													<td><?php echo($order['order_details']->installation==1)?'Yes':'No'?></td>
												<td><a href="{{url('/view_order_details/'.$order['order_details']->order_id)}}" class="btn "><i class="fa fa-eye padd_eyes" aria-hidden="true"></i></a></td>
											    <td>
											    ...
											    <td><a href="{{url('/track_order/'.$order['order_details']->order_id)}}" class="btn btn-primary2" data-toggle="tooltip" data-placement="bottom" title="" data-original-title="Track Order"><i class="fa fa-truck" aria-hidden="true"></i></a></td>
											    		<td>{{$order['order_details']->gstin}}</td>
											    		<td>{{$order['order_details']->comp_name}}</td>
											    		<td>{{$order['top']}}</td>
               	<td>{{$order['order_details']->address.' Email -'.$order['order_details']->email.' contact -'.$order['order_details']->mobile.'State - '.$order['order_details']->state_name.'City - '.$order['order_details']->city_name . ' - ' . $order['order_details']->pincode }}</td>
												</tr>
												<?php $i++; ?>
                                            @endforeach
                                            @endif
