						...
						<div class="page-header">
							...
							<div class="d-flex">
								<div class="justify-content-center">
									...
									<a href="{{url('add_slider')}}" class="btn btn-primary my-2 btn-icon-text">
									 Add Slider
									</a>
								</div>
							</div>
										...
										<div class="table-responsive">
											<table id="exportexample" class="table text-center table-bordered border-t0 key-buttons text-nowrap w-100" >
											 ...
											 @foreach($slider as $s)
												...
												<td>{{$i}}</td>
												<td>{{$s->name}}</td>
												<td><img src="{{url('/slider_image/'.$s->image)}}" width="50" height="50"></td>
											    <td>
										          <select onchange="change_status(<?php echo $s->id?>,this.value)">
											          <option value="1"<?php echo($s->slider_status==1)?'selected':''?>>Active</option>
											          <option value="0"<?php echo($s->slider_status==0)?'selected':''?>>In-active</option>
										          </select>
											     </td>
											     
												<td><a href="{{url('edit_slider/'.$s->id)}}" class="btn btn-primary">Edit</a></td>
                <td>
                    <form action="{{url('delete_slider/'.$s->id)}}" method="POST">
                        @csrf
                        @method('DELETE')
